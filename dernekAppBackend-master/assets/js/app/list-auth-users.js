...
$(document).ready(function() {
    $('.card-title-desc').text("Yetkili kullanıcılar getirildi, listeleniyor...");
    setTimeout(function() {
        $('#list-auth').show();
        $('#list-auth').DataTable({
            lengthMenu: [
            ...
            language: {
                ...
                zeroRecords: "Herhangi bir yetkili bulunamadı.",
                info: "_PAGES_ sayfa arasında _PAGE_. sayfa gösteriliyor.",
                infoEmpty: "Yetkili bulunamadı.",
                infoFiltered: "(_MAX_ yetkili arasında)",
                search: "Ara:",
                paginate: {
                    first: "İlk",
                    last: "Son",
        ...
        $('.card-title-desc').text("Aşağıda listelenen yetkilileri isterseniz silebilir, düzenleyebilir veya belirtilen formatlarda çıktı olarak dosya halinde alabilirsiniz.");
        //hideProgressBar();
    }, 400);
    ...
    $(document).on("click", ".DeleteAuth", function(e) {
        ...
        Swal.fire({
            title: 'Emin misiniz?',
            text: "Seçtiğiniz yetkili (no: " + auth_id + ") silinecek. Bu işlem geri alınamaz!",
            icon: 'warning',
            ...
            if (result.value) {
                var url = 'delete-authorized-user?id=' + auth_id;
                $.ajax({
                    method: "GET",
                    url: url,
